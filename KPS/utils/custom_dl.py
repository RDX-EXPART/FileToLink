...
class ByteStreamer:
    ...
    async def stream_file(self, message_id: int, offset: int = 0, limit: int = 0) -> AsyncGenerator[bytes, None]:
        ...
        # Include the bytes skipped inside the first Telegram chunk; without
        # this, some Range requests can end one chunk too early.
        leading_skip = offset % chunk_size
        chunk_limit = (leading_skip + limit + chunk_size - 1) // chunk_size if limit > 0 else 0
